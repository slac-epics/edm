...
mz_xyGraphClass *xyo = (mz_xyGraphClass *) client;
...
int yi;
  ...
  if ( w == xyo->pbAutoScale ) {
    ...
    for ( yi=0; yi<mz_xyGraphClass::NUM_Y_AXES; yi++ ) {
    ...
    if ( !xyo->eBuf ) {
      xyo->eBuf = new mz_xyGraphClass::editBufType;
    }
...
mz_xyGraphClass *xyo = (mz_xyGraphClass *) userarg;
...
mz_xyGraphClass *xyo = (mz_xyGraphClass *) ptr->objPtr;
...
static void yMonitorConnectionHOPR (
                ProcessVariable *pv,
                void *userarg )
{
        // All we do here is to add a ca monitor using add_value_callback
        objPlusIndexPtr ptr = (objPlusIndexPtr) userarg;

        if ( pv->is_valid() ) {
                pv->add_value_callback( yValueUpdateHOPR, userarg );
        }
}


static void yValueUpdateHOPR(
                ProcessVariable *pv,
                void *userarg )
{
        objPlusIndexPtr ptr = (objPlusIndexPtr) userarg;
        mz_xyGraphClass *xyo = (mz_xyGraphClass *) ptr->objPtr;
        int i = ptr->index - XYGC_K_MAX_TRACES;
        xyo->dbYMax[i] = pv->get_double();
}

static void yMonitorConnectionLOPR (
                ProcessVariable *pv,
                void *userarg )
{
        // All we do here is to add a ca monitor using add_value_callback
        ...
        if ( pv->is_valid() ) {
                pv->add_value_callback( yValueUpdateLOPR, userarg );
        }
}


static void yValueUpdateLOPR(
                ProcessVariable *pv,
                void *userarg )
{
        objPlusIndexPtr ptr = (objPlusIndexPtr) userarg;
    ...
    if ( xyo->forceVector[i] || ( xyo->yPvCount[i] > 1 ) ) { // vector
      ...
      if ( xyo->plotUpdateMode[i] != XYGC_K_UPDATE_ON_TRIG ) {
        ...
        xyo->actWin->addDefExeNode( xyo->aglPtr );
                                ...
                                if ( xyo->y1AxisSource[i] == XYGC_K_FROM_PV ) { 
                                    xyo->needY1Rescale[i] = 1;
                                    xyo->kpCancelMinY1[i] = 1;
...
mz_xyGraphClass *axygo = (mz_xyGraphClass *) client;
...
mz_xyGraphClass::mz_xyGraphClass ( void ) {
...
time_t t1, t2;
  ...
  name = new char[strlen("mzxyGraphClass")+1];
  strcpy( name, "mzxyGraphClass" );
...
// copy constructor
mz_xyGraphClass::mz_xyGraphClass
 ( const mz_xyGraphClass *source ) {
...
int mz_xyGraphClass::getDbXMinXMax (
  double *min,
  double *max
) {
...
void mz_xyGraphClass::getDbYMinYMax (
  double *min,
  double *max,
  int yi
) {
...
void mz_xyGraphClass::getXMinMax (
  double *min,
  double *max
) {
...
void mz_xyGraphClass::getYMinMax (
  int yi,
  double min[],
  double max[]
) {
...
int mz_xyGraphClass::createInteractive (
...
int mz_xyGraphClass::createFromFile (
  FILE *f,
  char *name,
...
int mz_xyGraphClass::old_createFromFile (
  FILE *f,
  char *name,
  activeWindowClass *_actWin )
{
...
int mz_xyGraphClass::importFromXchFile (
  FILE *f,
  char *name,
  activeWindowClass *_actWin )
{
...
int mz_xyGraphClass::genericEdit ( void ) {

char title[32], *ptr;
int i, yi;

  ptr = actWin->obj.getNameFromClass( "mzxyGraphClass" );
  if ( ptr )
...
int mz_xyGraphClass::editCreate ( void ) {
...
void mz_xyGraphClass::regenBuffer ( void ) {
...
void mz_xyGraphClass::genChronoVector (
  int i, // trace
  int *rescale
) {
...
void mz_xyGraphClass::genXyVector (
  int i, // trace
  int *rescale
) {
...
int mz_xyGraphClass::fullRefresh ( void ) {
...
int mz_xyGraphClass::erase ( void ) {
...
void mz_xyGraphClass::drawCircles (
  int index,
  XPoint *xp,
  int n
) {
...
void mz_xyGraphClass::drawSquares (
  int index,
  XPoint *xp,
  int n
) {
...
void mz_xyGraphClass::drawDiamonds (
  int index,
  XPoint *xp,
  int n
) {
...
int mz_xyGraphClass::eraseActive ( void ) {
...
int mz_xyGraphClass::drawActiveOne (
  int i // trace
) {
...
int mz_xyGraphClass::drawActive ( void ) {
...
void mz_xyGraphClass::bufInvalidate ( void ) {
...
int mz_xyGraphClass::expandTemplate (
  int numMacros,
  char *macros[],
  char *expansions[] ) {
...
int mz_xyGraphClass::containsMacros ( void ) {
...
char *mz_xyGraphClass::firstDragName ( void ) {
...
char *mz_xyGraphClass::nextDragName ( void ) {
...
char *mz_xyGraphClass::dragValue (
  int i
) {
	    ...
	    if ( yPv[i] ) {
	      yPv[i]->add_conn_state_callback( yMonitorConnection,
               &ycArgRec[i] );
                                                                ...
                                                                char hoprPVName[256];
                                                                char loprPVName[256];
                                                                strncpy(hoprPVName, yPvExpStr[i].getExpanded(), 256);
                                                                strncpy(loprPVName, yPvExpStr[i].getExpanded(), 256);
                                                                strcat(hoprPVName, ".HOPR");
                                                                strcat(loprPVName, ".LOPR");
                                                                yPvHOPR[i] = the_PV_Factory->create( hoprPVName);
                                                                yPvLOPR[i] = the_PV_Factory->create( loprPVName);
                                                                if(yPvHOPR[i]) { 
                                                                        yPvHOPR[i]->add_conn_state_callback( yMonitorConnectionHOPR, &ycArgRec[i] );
                                                                }
                                                                if(yPvLOPR[i]) { 
                                                                        yPvLOPR[i]->add_conn_state_callback( yMonitorConnectionLOPR, &ycArgRec[i] );
                                                                }
	    }
	    else {
              fprintf( stderr, "pv create failed for [%s]\n",
...
int mz_xyGraphClass::deactivate (
  int pass
) {
...
int i;
  ...
  case 1:
    ...
    for ( i=0; i<numTraces; i++ ) {
      ...
      if(yPvHOPR[i]) { 
        yPvHOPR[i]->remove_conn_state_callback(yMonitorConnectionHOPR, &ycArgRec[i]);
        yPvHOPR[i]->remove_value_callback(yValueUpdateHOPR, &ycArgRec[i]);
        yPvHOPR[i]->release();
        yPvHOPR[i] = NULL;
      }

      if(yPvLOPR[i]) { 
        yPvLOPR[i]->remove_conn_state_callback(yMonitorConnectionLOPR, &ycArgRec[i]);
        yPvLOPR[i]->remove_value_callback(yValueUpdateLOPR, &ycArgRec[i]);
        yPvLOPR[i]->release();
        yPvLOPR[i] = NULL;
      }
...
void mz_xyGraphClass::updateDimensions ( void ) {
...
int mz_xyGraphClass::getButtonActionRequest (
  int *up,
  int *down,
  int *drag )
{
...
void mz_xyGraphClass::executeDeferred ( void ) {
      ...
      if ( yPv[i] ) {
        ...
        if ( yPv[i]->is_valid() ) {
          ...
          yPvSize[i] = yPvDim[i] * (int) yPv[i]->get_specific_type().size / 8;
                                                        dbYMin[i] = yPvLOPR[i]->get_double();
                                                        dbYMax[i] = yPvHOPR[i]->get_double();
...
void mz_xyGraphClass::initPlotInfo (
  int trace
) {
...
int mz_xyGraphClass::fillPlotArray (
  int trace,
  int isVector
) {
...
int mz_xyGraphClass::fillScalarPlotArray (
  int trace
) {
...
int mz_xyGraphClass::fillVectorPlotArray (
  int trace
) {
...
void mz_xyGraphClass::drawBorder ( void ) {
...
void mz_xyGraphClass::drawXScale ( void ) {
...
void mz_xyGraphClass::drawY1Scale ( void ) {
...
void mz_xyGraphClass::drawY2Scale ( void ) {
...
void mz_xyGraphClass::drawGrid ( void ) {
...
void mz_xyGraphClass::drawTitle ( void ) {
...
void mz_xyGraphClass::drawXlabel ( void ) {
...
void mz_xyGraphClass::drawYlabel ( void ) {
...
void mz_xyGraphClass::drawY2label ( void ) {
...
char *mz_xyGraphClass::getSearchString (
  int i
) {
...
void mz_xyGraphClass::replaceString (
  int i,
  int max,
  char *string
) {
...
char *mz_xyGraphClass::crawlerGetFirstPv ( void ) {
...
char *mz_xyGraphClass::crawlerGetNextPv ( void ) {
...
void *create_mz_xyGraphClassPtr ( void ) {
